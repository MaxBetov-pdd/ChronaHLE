...
impl Mem {
    ...
    pub fn set_null_segment_size(&mut self, new_null_segment_size: VAddr) {
        ...
        self.vm_allocator
            ...
            .unwrap();
        if new_null_segment_size != 0 {
            unsafe {
                host::protect_no_access(self.bytes.cast(), new_null_segment_size as usize)
                    .expect("Failed to protect the guest null segment");
            }
        }
    ...
    pub fn malloc_size_in_heap(
        ...
        heap.allocated_size(ptr.to_bits()).unwrap_or(0)
    }

    fn malloc_size_in_heap_or_panic(
        &mut self,
        heap: Option<&mut HeapAllocator>,
        ptr: ConstVoidPtr,
    ) -> GuestUSize {
    ...
    pub fn realloc_in_heap(
        ...
        //       to realloc inplace
        let old_size = self.malloc_size_in_heap_or_panic(heap.as_deref_mut(), old_ptr.cast_const());
        if old_size >= size {
            return old_ptr;
        }
