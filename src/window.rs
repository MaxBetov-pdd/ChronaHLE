...
#[derive(Default)]
struct RgbaPresentationResources {
    texture: u32,
    width: u32,
    height: u32,
}
...
pub struct Window {
    ...
    rgba_presentation: RgbaPresentationResources,
...
impl Window {
    ...
    pub fn new(
        ...
        let mut window = Window {
            ...
            rgba_presentation: RgbaPresentationResources::default(),
    ...
    /// Present bottom-to-top RGBA pixels through the internal GLES1 renderer.
    /// This is the compatibility presentation path for native GLES2 contexts
    /// whose framebuffer cannot be consumed by the GLES1 compositor directly.
    pub fn present_rgba_pixels(&mut self, pixels: &[u8], width: u32, height: u32) {
        let expected_len = (width as usize)
            .checked_mul(height as usize)
            .and_then(|size| size.checked_mul(4))
            .expect("RGBA frame dimensions overflow");
        assert_eq!(pixels.len(), expected_len);

        let viewport = self.viewport();
        let rotation_matrix = self.rotation_matrix();
        let virtual_cursor_visible_at = self.virtual_cursor_visible_at();
        ...
        unsafe {
            ...
            if self.rgba_presentation.texture == 0 {
                gl_ctx.GenTextures(1, &mut self.rgba_presentation.texture);
            }
            gl_ctx.BindTexture(gles11::TEXTURE_2D, self.rgba_presentation.texture);
            if self.rgba_presentation.width != width || self.rgba_presentation.height != height {
                gl_ctx.TexImage2D(
                    gles11::TEXTURE_2D,
                    0,
                    gles11::RGBA as _,
                ...
                self.rgba_presentation.width = width;
                self.rgba_presentation.height = height;
            } else {
                gl_ctx.TexSubImage2D(
                    gles11::TEXTURE_2D,
                    0,
                    0,
                    0,
...
pub fn show_error_messagebox(window: Option<&Window>, error_message: &str) {
    ...
    let Ok(clicked_button) = messagebox::show_message_box(
        ...
        &mbox,
        "ChronaHLE crashed!",
        &format!("ChronaHLE crashed with the following error: {error_message}"),
