...
fn is_texture_binding_target(target: GLenum) -> bool {
    target == gl21::TEXTURE_2D || target == gl21::TEXTURE_CUBE_MAP
}

fn is_texture_image_target(target: GLenum) -> bool {
    target == gl21::TEXTURE_2D
        || target == gl21::TEXTURE_CUBE_MAP_POSITIVE_X
        || target == gl21::TEXTURE_CUBE_MAP_NEGATIVE_X
        || target == gl21::TEXTURE_CUBE_MAP_POSITIVE_Y
        || target == gl21::TEXTURE_CUBE_MAP_NEGATIVE_Y
        || target == gl21::TEXTURE_CUBE_MAP_POSITIVE_Z
        || target == gl21::TEXTURE_CUBE_MAP_NEGATIVE_Z
}
...
const GET_PARAMS: ParamTable = ParamTable(&[
    ...
    // Shader/program limits from OpenGL ES 2.0, backed by OpenGL 2.x.
    (gl21::CURRENT_PROGRAM, ParamType::Int, 1),
    (gl21::MAX_COMBINED_TEXTURE_IMAGE_UNITS, ParamType::Int, 1),
    (gl21::MAX_DRAW_BUFFERS, ParamType::Int, 1),
    ...
    (gl21::TEXTURE_BINDING_CUBE_MAP, ParamType::Int, 1),
...
fn load_gl_functions<F>(native_gles2: bool, mut loader: F)
where
    F: FnMut(&'static str) -> *const std::ffi::c_void,
{
    if native_gles2 {
        gles20::load_with(&mut loader);
    }
    gl21::load_with(|name| {
        let address = loader(name);
        if !native_gles2 || !address.is_null() {
            return address;
        }

        // OpenGL ES 2 promotes framebuffer objects to core and exposes a few
        // desktop ARB entry points through OES names. The signatures match.
...
impl GLES1OnGL2Context {
    /// Construct the programmable subset on a native EGL OpenGL ES 2 context.
    /// GLES1-only entry points remain unused for this context.
    #[cfg(target_os = "android")]
    pub fn new_native_gles2(window: &mut Window) -> Result<Self, String> {
        Ok(Self {
...
impl GLESContext for GLES1OnGL2Context {
    ...
    fn make_current<'gl_ctx, 'win: 'gl_ctx>(
        ...
        if self.gl_ctx.is_current() && self.is_loaded {
            return Box::new(GLES1OnGL2 {
                ...
                native_gles2: self.native_gles2,
            });
        }
        ...
        load_gl_functions(self.native_gles2, |s| window.gl_get_proc_address(s));
        self.is_loaded = true;
    ...
    unsafe fn make_current_unchecked_for_window<'gl_ctx>(
        ...
        load_gl_functions(self.native_gles2, loader_fn);
        self.is_loaded = true;
...
impl GLES for GLES1OnGL2<'_> {
    ...
    unsafe fn GetBooleanv(&mut self, pname: GLenum, params: *mut GLboolean) {
        if !GET_PARAMS.contains(pname) {
            log_dbg!("Passing unknown glGetBooleanv({pname:#x}) through to OpenGL");
    ...
    unsafe fn GetFloatv(&mut self, pname: GLenum, params: *mut GLfloat) {
        if !GET_PARAMS.contains(pname) {
            log_dbg!("Passing unknown glGetFloatv({pname:#x}) through to OpenGL");
    ...
    unsafe fn GetIntegerv(&mut self, pname: GLenum, params: *mut GLint) {
        if !GET_PARAMS.contains(pname) {
            log_dbg!("Passing unknown glGetIntegerv({pname:#x}) through to OpenGL");
    ...
    unsafe fn BlendFuncSeparate(
        ...
        let common_factors = [
            ...
            gl21::ONE_MINUS_DST_ALPHA,
            gl21::CONSTANT_COLOR,
            gl21::ONE_MINUS_CONSTANT_COLOR,
            gl21::CONSTANT_ALPHA,
            gl21::ONE_MINUS_CONSTANT_ALPHA,
        ];
        let sfactors = [
            gl21::DST_COLOR,
            gl21::ONE_MINUS_DST_COLOR,
        ...
        let dfactors = [gl21::SRC_COLOR, gl21::ONE_MINUS_SRC_COLOR];
        assert!(
            common_factors.contains(&sfactor_rgb)
                || sfactors.contains(&sfactor_rgb)
                || dfactors.contains(&sfactor_rgb)
        );
        assert!(
            common_factors.contains(&dfactor_rgb)
                || sfactors.contains(&dfactor_rgb)
                || dfactors.contains(&dfactor_rgb)
        );
        assert!(
            common_factors.contains(&sfactor_alpha)
                || sfactors.contains(&sfactor_alpha)
                || dfactors.contains(&sfactor_alpha)
        );
        assert!(
            common_factors.contains(&dfactor_alpha)
                || sfactors.contains(&dfactor_alpha)
                || dfactors.contains(&dfactor_alpha)
        );
    ...
    unsafe fn BlendEquationSeparate(&mut self, mode_rgb: GLenum, mode_alpha: GLenum) {
        ...
        assert!(functions.contains(&mode_rgb));
        assert!(functions.contains(&mode_alpha));
    ...
    unsafe fn ShaderSource(
        ...
        gl21::ShaderSource(shader, count, string, length)
    }
    ...
    unsafe fn GetShaderSource(
        ...
        gl21::GetShaderSource(shader, buf_size, length, source)
    }
    ...
    unsafe fn GetActiveUniform(
        ...
        gl21::GetActiveUniform(program, index, buf_size, length, size, type_, name)
    }
    ...
    unsafe fn BindTexture(&mut self, target: GLenum, texture: GLuint) {
        assert!(is_texture_binding_target(target));
    ...
    unsafe fn TexImage2D(
        ...
        assert!(is_texture_image_target(target));
        assert!(level >= 0);
        assert!(
