...
impl GLES for GLES1Native<'_> {
    ...
    unsafe fn BlendFuncSeparate(
        ...
        if sfactor_rgb != sfactor_alpha || dfactor_rgb != dfactor_alpha {
            log_dbg!(
                "Approximating glBlendFuncSeparate({:#x}, {:#x}, {:#x}, {:#x}) with glBlendFunc",
                sfactor_rgb,
                dfactor_rgb,
                sfactor_alpha,
                dfactor_alpha
        ...
        gles11::BlendFunc(sfactor_rgb, dfactor_rgb)
    }
    ...
    unsafe fn BlendEquationSeparate(&mut self, mode_rgb: GLenum, mode_alpha: GLenum) {
        if mode_rgb != mode_alpha {
            log_dbg!(
                "Approximating glBlendEquationSeparate({:#x}, {:#x}) with glBlendEquationOES",
                mode_rgb,
                mode_alpha
            );
        }
        gles11::BlendEquationOES(mode_rgb);
    }
    unsafe fn ColorMask(
        &mut self,
        red: GLboolean,
    ...
    // OpenGL ES 2.0 shader/program entry points are not available through the
    // native ES 1.1 backend.
    unsafe fn CreateShader(&mut self, _type_: GLenum) -> GLuint {
        unimplemented!("OpenGL ES 2.0 is not supported by GLES1Native")
    }
    ...
    unsafe fn BindVertexArrayOES(&mut self, _array: GLuint) {
        unimplemented!("OpenGL ES 2.0/OES_vertex_array_object on native ES1 backend")
    }
