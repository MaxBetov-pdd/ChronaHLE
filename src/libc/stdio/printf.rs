...
fn format_decimal_integer(
    ...
    let mut digits = if magnitude == 0 && precision == Some(0) {
        String::new()
    } else {
        magnitude.to_string()
    };
    ...
    let content_len = digits.len() + usize::from(sign.is_some());
    let width = usize::try_from(pad_width).unwrap();
    let padding = width.saturating_sub(content_len);
    let mut formatted = String::with_capacity(content_len + padding);
    if !left_justified && (pad_char != '0' || precision.is_some()) {
        formatted.push_str(&" ".repeat(padding));
    }
    if let Some(sign) = sign {
        formatted.push(sign);
    }
    if !left_justified && pad_char == '0' && precision.is_none() {
        formatted.push_str(&"0".repeat(padding));
    }
    ...
    loop {
        ...
        match specifier {
            ...
            b'd' | b'i' | b'u' => {
                ...
                let formatted = if specifier == b'u' {
