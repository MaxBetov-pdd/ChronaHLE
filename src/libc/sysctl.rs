...
static SYSCTL_VALUES: [(SysCtlNamePath, &str, SysInfoType); 24] = [
    // Generic CPU, I/O
    ...
    (SysCtlNamePath::Undefined, "hw.activecpu", SysInfoType::Int32(1)),
    (SysCtlNamePath::Undefined, "hw.physicalcpu", SysInfoType::Int32(1)),
    (SysCtlNamePath::Undefined, "hw.physicalcpu_max", SysInfoType::Int32(1)),
    (SysCtlNamePath::Undefined, "hw.logicalcpu", SysInfoType::Int32(1)),
    (SysCtlNamePath::Undefined, "hw.logicalcpu_max", SysInfoType::Int32(1)),
...
fn sysctl(
    ...
    match name_len {
        ...
        6 => {
            let mib: Vec<i32> = (0..6)
                .map(|idx| env.mem.read(name + idx as GuestUSize))
                .collect();
            if mib[0] == CTL_NET && mib[1] == PF_ROUTE {
                if !oldlenp.is_null() {
                    env.mem.write(oldlenp, 0);
                }
                log!(
                    "TODO: sysctl() routing query {:?}, returning an empty route table",
                    mib
                );
                0
            } else {
                set_errno(env, ENOENT);
                log!("Unknown six-component sysctl MIB {:?}, returning -1", mib);
                -1
            }
        }
    ...
    F: FnOnce(&mut Environment) -> (&'static str, SysInfoType),
{
    ...
    let len: GuestUSize = match val {
        ...
        SysInfoType::Timeval(_) => guest_size_of::<timeval>(),
        _ => unimplemented!(),
    };
    if oldp.is_null() {
    ...
    match val {
        ...
        SysInfoType::Timeval(timeval) => {
            env.mem.write(oldp.cast(), timeval);
        }
        _ => unimplemented!(),
    }
    env.mem.write(oldlenp, len);
