...
impl GuestArg for in_addr {
    const REG_COUNT: usize = <in_addr_t as GuestArg>::REG_COUNT;

    fn from_regs(regs: &[u32]) -> Self {
        Self {
            s_addr: <in_addr_t as GuestArg>::from_regs(regs),
        }
    }

    fn to_regs(self, regs: &mut [u32]) {
        <in_addr_t as GuestArg>::to_regs(self.s_addr, regs)
    }
}

#[derive(Default)]
pub struct State {
    inet_ntoa_buffer: Option<MutPtr<u8>>,
}
...
fn inet_ntoa(env: &mut Environment, in_: in_addr) -> MutPtr<u8> {
    let ipv4_addr = Ipv4Addr::from_bits(u32::from_be(in_.s_addr));
    let addr_string = ipv4_addr.to_string();
    log_dbg!("inet_ntoa({:?}) => {}", in_, addr_string);

    if let Some(old_buffer) = env.libc_state.arpa_inet.inet_ntoa_buffer.take() {
        env.mem.free(old_buffer.cast());
    }

    let buffer = env.mem.alloc_and_write_cstr(addr_string.as_bytes());
    env.libc_state.arpa_inet.inet_ntoa_buffer = Some(buffer);
    buffer
}
