...
pub fn open_direct(env: &mut Environment, path: ConstPtr<u8>, flags: i32) -> FileDescriptor {
    ...
    if path_string == "/dev/null" {
        let host_object = PosixFileHostObject {
            file: GuestFile::NullDevice,
            needs_flush: false,
            reached_eof: false,
            fd_flags: 0,
            open_flags: flags & (O_ACCMODE | O_NONBLOCK | O_APPEND),
        };
        return find_or_create_fd(env, host_object);
    }
    if path_string == "/dev/random" || path_string == "/dev/urandom" {
        let host_object = PosixFileHostObject {
            file: GuestFile::RandomDevice(0x12345678),
            needs_flush: false,
            reached_eof: false,
            fd_flags: 0,
            open_flags: flags & (O_ACCMODE | O_NONBLOCK | O_APPEND),
...
pub fn read(
    ...
    match file.file.read(buffer_slice) {
        Ok(bytes_read) => {
            if bytes_read < buffer_slice.len() && size != 0 {
                file.reached_eof = true;
            }
            if bytes_read < buffer_slice.len() {
                log_dbg!(
                    "read({:?}, {:?}, {:#x}) reached EOF after {:#x} bytes",
                    fd,
                    buffer,
                    size,
                    bytes_read,
...
pub fn write(
    ...
    let standard_write_result = match fd {
        STDOUT_FILENO => Some(std::io::stdout().write(buffer_slice)),
        STDERR_FILENO => Some(std::io::stderr().write(buffer_slice)),
        _ => None,
    };
    if let Some(result) = standard_write_result {
        return match result {
            Ok(bytes_written) => bytes_written.try_into().unwrap(),
            Err(error) => {
                log!(
                    "Warning: write({:?}, {:?}, {:#x}) to standard fd encountered error {:?}, returning -1",
                    fd,
                    buffer,
                    size,
                    error,
...
fn fcntl(
    ...
    let is_standard_fd = matches!(fd, STDIN_FILENO | STDOUT_FILENO | STDERR_FILENO);
    if fd < 0
        || (!is_standard_fd
            && env
                .libc_state
    ...
    match cmd {
        ...
        F_SETFL => {
            let flags: OpenFlag = args.start().next(env);
            let unsupported = flags & !(O_ACCMODE | O_NONBLOCK | O_APPEND);
            if unsupported != 0 {
                log!(
                    "Warning: fcntl({}, F_SETFL, {:#x}) ignoring unsupported open flags {:#x}",
                    fd,
                    flags,
                    unsupported
                );
            ...
            if is_standard_fd {
                log_dbg!(
                    "TODO: fcntl({}, F_SETFL, {:#x}) ignored for standard file descriptor",
                    fd,
                    flags
                );
                return 0;
            ...
            let file = env.libc_state.posix_io.file_for_fd(fd).unwrap();
            file.open_flags = (file.open_flags & O_ACCMODE) | (flags & (O_NONBLOCK | O_APPEND));
        }
        F_GETLK => {
