...
//!
//! See also [crate::paths], which has paths for host files used by ChronaHLE.
...
enum FileLocation {
    ...
    /// Name of a resource file bundled with ChronaHLE. Read only.
...
impl GuestPath {
    ...
    ///
    /// Warning! This function should only be used for internal ChronaHLE
    /// purposes.
...
fn handle_open_err<T, E: std::fmt::Display, P: std::fmt::Debug>(
    ...
    match open_result {
        Ok(ok) => ok,
        Err(e) => panic!("Unexpected I/O failure when trying to access real path {host_path:?}: {e}. This might indicate that files needed by ChronaHLE are missing, or were moved while it was running."),
    }
}
...
impl GuestFile {
    ...
    pub fn sync_all(&self) -> std::io::Result<()> {
        match self {
            ...
            | GuestFile::NullDevice
            | GuestFile::RandomDevice(_)
    ...
    pub fn set_len(&self, len: u64) -> std::io::Result<()> {
        match self {
            ...
            GuestFile::NullDevice => Ok(()),
            GuestFile::RandomDevice(_) => Err(std::io::Error::new(
                std::io::ErrorKind::Unsupported,
                "Resize operation not supported on random device",
            )),
    ...
    pub fn is_seekable(&self) -> bool {
        ...
        !matches!(
            self,
            GuestFile::NullDevice | GuestFile::RandomDevice(_) | GuestFile::Socket
        )
    }
}
...
impl Read for GuestFile {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        match self {
            ...
            GuestFile::NullDevice => Ok(0),
            GuestFile::RandomDevice(state) => {
...
impl Write for GuestFile {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        match self {
            ...
            GuestFile::NullDevice => Ok(buf.len()),
            GuestFile::RandomDevice(_) => Err(std::io::Error::new(
                std::io::ErrorKind::Unsupported,
                "Write operation not supported on random device",
            )),
...
impl Seek for GuestFile {
    fn seek(&mut self, pos: std::io::SeekFrom) -> std::io::Result<u64> {
        match self {
            ...
            GuestFile::RandomDevice(_) => Err(std::io::Error::new(
                std::io::ErrorKind::Unsupported,
                "Seek operation not supported on random device",
            )),
            _ => unimplemented!(),
        }
    }
...
impl Fs {
    ...
    pub fn new(
        ...
        if !read_only_mode {
            // Special case: Some apps expect common Library subdirectories to
            // exist from the start, as they do in a real iOS sandbox.
            for subdir in ["Preferences", "Caches", "Application Support"] {
                let path = paths::user_data_base_path()
                    .join(paths::SANDBOX_DIR)
                    .join(bundle_id)
                    .join("Library")
        ...
        // Some Free Software libraries are bundled with ChronaHLE.
