...
/// Create an OpenGL ES 2.0 context backed by the OpenGL 2.1 compatibility
/// implementation.
pub fn create_gles2_ctx(env: &mut Environment) -> Box<dyn GLESContext> {
    env.on_parent_stack_in_coroutine(|window, _options| {
        assert!(window.on_main_stack());
        #[cfg(target_os = "android")]
        log!("Creating a native OpenGL ES 2.0 context:");
        #[cfg(not(target_os = "android"))]
        log!("Creating an OpenGL ES 2.0 context using OpenGL 2.1 compatibility:");

        #[cfg(target_os = "android")]
        let result = GLES1OnGL2Context::new_native_gles2(window);
        #[cfg(not(target_os = "android"))]
        let result = GLES1OnGL2Context::new(window);
