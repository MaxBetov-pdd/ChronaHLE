...
impl Environment {
    ...
    pub fn new(
        ...
        let mut dylibs_to_load = executable.dynamic_libraries.clone();
        let mut visited_dylib_paths = HashSet::new();
        let mut next_dylib = 0;
        while next_dylib < dylibs_to_load.len() {
            let dylib = dylibs_to_load[next_dylib].clone();
            next_dylib += 1;
            if !visited_dylib_paths.insert(dylib.clone()) {
                continue;
            }
            ...
            if fs.is_file(dylib_path) {
                ...
                let dylib = mach_o::MachO::load_from_file(dylib_path, &fs, &mut mem, dylib_slide)
                    .map_err(|e| format!("Could not load bundled dylib: {e}"))?;
                dylibs_to_load.extend(dylib.dynamic_libraries.iter().cloned());
                dylibs.push(dylib);
    ...
    fn dump_memory_error_context(&self) {
        let regs = self.cpu.regs();
        for &(label, addr, count) in &[
            ("r0", regs[0], 0x40),
            ("r1", regs[1], 0x40),
            ("r2", regs[2], 0x20),
            ("sp", regs[cpu::Cpu::SP], 0x100),
        ] {
            let ptr = mem::Ptr::from_bits(addr);
            let Some(bytes) = self.mem.get_bytes_fallible(ptr, count) else {
                echo_no_panic!("Could not dump memory at {label}={addr:#010x}");
                continue;
            };

            echo_no_panic!("Memory dump at {label}={addr:#010x} ({count} bytes):");
            for (idx, chunk) in bytes.chunks(16).enumerate() {
                let mut line = format!("{:#010x}:", addr + (idx as u32 * 16));
                for byte in chunk {
                    use std::fmt::Write;
    ...
    fn debug_cpu_error(&mut self, error: cpu::CpuError) {
        ...
        if self.gdb_server.is_none() {
            if matches!(error, cpu::CpuError::MemoryError) {
                echo!("Register state at CPU memory error:");
                self.dump_all_regs();
                self.dump_memory_error_context();
            }
    ...
    fn set_up_initial_env_vars(&mut self) {
        ...
        let mut insert_env = |name: &[u8], value: &[u8]| {
            let value_cstr = self.mem.alloc_and_write_cstr(value);
            self.env_vars.insert(name.to_vec(), value_cstr);
        };

        insert_env(b"HOME", self.fs.home_directory().as_str().as_bytes());
        insert_env(b"LANG", b"en_US.UTF-8");
        insert_env(b"LC_ALL", b"en_US.UTF-8");
        insert_env(b"MONO_DISABLE_MANAGED_COLLATION", b"yes");
        insert_env(b"MONO_COLLATION_QUICK_CHECK_DISABLED", b"yes");
    }
