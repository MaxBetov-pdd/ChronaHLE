...
pub struct State {
    ...
    /// Root view controller for each window. Strong references.
...
- (())dealloc {
    ...
    let root_view_controller = {
        let roots = &mut env.framework_state.uikit.ui_view.ui_window.root_view_controllers;
        roots
            .iter()
            .position(|&(window, _)| window == this)
            .map(|idx| roots.remove(idx).1)
    };
    if let Some(root_view_controller) = root_view_controller {
...
- (id)rootViewController {
    env.framework_state
        ...
        .iter()
        .find(|&&(window, _)| window == this)
        .map_or(nil, |&(_, view_controller)| view_controller)
}

- (())setRootViewController:(id)root_view_controller {
    let old_root_view_controller: id = msg![env; this rootViewController];
    if old_root_view_controller == root_view_controller {
        return;
    }

    if root_view_controller != nil {
        retain(env, root_view_controller);
    }

    {
        ...
        if let Some(idx) = roots.iter().position(|&(window, _)| window == this) {
            if root_view_controller == nil {
                ...
                roots[idx].1 = root_view_controller;
            }
        } else if root_view_controller != nil {
            roots.push((this, root_view_controller));
        }
    }

    if old_root_view_controller != nil {
        let old_view: id = msg![env; old_root_view_controller view];
        () = msg![env; old_view removeFromSuperview];
        release(env, old_root_view_controller);
    }

    if root_view_controller != nil {
        let view: id = msg![env; root_view_controller view];
...
- (())addSubview:(id)view {
    ...
    let device_orientation = env
        .window
        .as_ref()
        .map_or(crate::window::DeviceOrientation::Portrait, |window| {
            window.current_rotation()
        });

    if let Some(orientation) = match device_orientation {
