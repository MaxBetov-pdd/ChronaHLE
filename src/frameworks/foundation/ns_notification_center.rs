...
use crate::objc::{
    copy_block, id, msg, msg_class, msg_send, nil, objc_classes, release, release_block, retain,
...
impl Observer {
    fn observer(&self) -> id {
        match *self {
            Observer::Selector { observer, .. } | Observer::Block { observer, .. } => observer,
        }
    }

    fn object(&self) -> id {
        match *self {
            Observer::Selector { object, .. } | Observer::Block { object, .. } => object,
        }
    }
}
...
struct BlockObserverHostObject;
impl HostObject for BlockObserverHostObject {}
...
- (())addObserver:(id)observer
    ...
    let host_obj = env.objc.borrow_mut::<NSNotificationCenterHostObject>(this);
    host_obj.observers.entry(name).or_default().push(Observer::Selector {
        observer,
        selector,
        object,
    });
...
- (id)addObserverForName:(NSNotificationName)name
    ...
    log_dbg!(
        "[(NSNotificationCenter*){:?} addObserverForName:{:?} object:{:?} queue:nil usingBlock:{:?}]",
        this,
        name,
        object,
        block,
    ...
    retain(env, object);
    let block = copy_block(env, block);

    let nsobject = env.objc.get_known_class("NSObject", &mut env.mem);
    let observer = env.objc.alloc_object(nsobject, Box::new(BlockObserverHostObject), &mut env.mem);
    let host_obj = env.objc.borrow_mut::<NSNotificationCenterHostObject>(this);
    host_obj.observers.entry(name).or_default().push(Observer::Block {
        observer,
        block,
        object,
    });
...
- (())postNotification:(id)notification {
    ...
    for observer_entry in observers {
        ...
        let object = observer_entry.object();
        if object != nil && notification_poster != object {
            continue;
        }

        match observer_entry {
