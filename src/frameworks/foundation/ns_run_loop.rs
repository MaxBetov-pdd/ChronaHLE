...
- (())addTimer:(id)timer // NSTimer*
    ...
    if !(msg![env; mode isEqualToString:default_mode]
        || msg![env; mode isEqualToString:common_modes])
    {
        log_dbg!(
            "Tolerating addTimer:forMode: with unsupported mode {:?}",
...
/// Pump audio-related main run loop sources once without sleeping.
///
/// Some games, particularly engines with their own main loop, can keep running
/// guest code without explicitly iterating `NSRunLoop`. Audio queue callbacks
/// are still run loop sources, so service them from the emulator scheduler too.
pub fn pump_main_run_loop_audio_queue_sources(env: &mut Environment) {
    if env.current_thread != 0 {
        return;
    }

    let run_loop = env.threads[0]
        .framework_state
        .foundation
        .ns_run_loop
        .run_loop;
...
/// Service RemoteIO units while a game-owned render loop is keeping the main
/// NSRunLoop busy. On iOS these callbacks run independently on the real-time
/// audio thread; touchHLE currently invokes them synchronously.
pub fn pump_main_run_loop_audio_unit_sources(env: &mut Environment) {
    if env.current_thread != 0 {
        return;
    }
