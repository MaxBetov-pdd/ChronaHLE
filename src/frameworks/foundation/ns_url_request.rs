...
struct NSURLRequestHostObject {
    ...
    /// `NSMutableDictionary*`, for properties attached by `NSURLProtocol`.
    protocol_properties: id,
}
...
+ (id)allocWithZone:(NSZonePtr)_zone {
    ...
    let protocol_properties: id = msg_class![env; NSMutableDictionary new];
...
- (())dealloc {
    ...
    release(env, protocol_properties);
...
pub(super) fn protocol_property(env: &mut crate::Environment, request: id, key: id) -> id {
    let properties = env
        .objc
        .borrow::<NSURLRequestHostObject>(request)
        .protocol_properties;
    msg![env; properties objectForKey:key]
}

pub(super) fn set_protocol_property(env: &mut crate::Environment, request: id, key: id, value: id) {
    let properties = env
        .objc
        .borrow::<NSURLRequestHostObject>(request)
        .protocol_properties;
    () = msg![env; properties setObject:value forKey:key];
}

pub(super) fn remove_protocol_property(env: &mut crate::Environment, request: id, key: id) {
    let properties = env
        .objc
        .borrow::<NSURLRequestHostObject>(request)
        .protocol_properties;
    () = msg![env; properties removeObjectForKey:key];
}
