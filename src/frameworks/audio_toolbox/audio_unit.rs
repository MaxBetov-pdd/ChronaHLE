...
fn AudioUnitSetProperty(
    ...
    match in_id {
        ...
        kAudioUnitProperty_StreamFormat => {
            ...
            log!(
                "AudioUnit {:?} stream format: scope={}, element={}, {:?}",
                in_unit,
                in_scope,
                in_element,
                stream_format
...
fn AudioOutputUnitStart(env: &mut Environment, ci: AudioUnit) -> OSStatus {
    ...
    audio_unit_state.al_source_primed = false;
    audio_unit_state.target_queued_buffers = 5;
...
pub fn render_audio_unit(env: &mut Environment, audio_unit: AudioUnit) {
    ...
    let mut al_source_primed = audio_unit_host_object.al_source_primed;
    ...
    unsafe {
        context.GetSourcei(al_source, AL_BUFFERS_QUEUED, &mut buffers_queued);
        assert_eq!(context.GetError(), 0);
    }

    let now = Instant::now();

    // RemoteIO is driven by a hardware-sized periodic buffer, independently
    // of the UI run loop. Keep a small queue ahead so a slow frame cannot
    // starve OpenAL and force the source to restart.
    let number_frames = (f64::from(current_hardware_io_buffer_duration) * sample_rate)
        .round()
        .clamp(
            1.0,
            f64::from(audio_unit_host_object.maximum_frames_per_slice),
        ) as u32;
    if !audio_unit_host_object.render_config_logged {
        log!(
            "AudioUnit {:?} render config: stream={:?}, input={:?}, output={:?}, hardware_rate={}, buffer_duration={:.6}s, frames={}, playback_rate={}",
            audio_unit,
            stream_format,
            input_stream_format,
            output_stream_format,
        ...
        audio_unit_host_object.render_config_logged = true;
    }
    let target_queued_buffers = audio_unit_host_object.target_queued_buffers;
    let buffers_to_render = target_queued_buffers.saturating_sub(buffers_queued);
    ...
    for _ in 0..buffers_to_render {
        ...
        unsafe {
            ...
            if al_source_state != AL_PLAYING {
                if al_source_primed {
                    log!(
                        "AudioUnit {:?} recovered from an OpenAL underrun",
                        audio_unit
                    );
                    had_underrun = true;
                }
                context.SourcePlay(al_source);
                al_source_primed = true;
            }
    ...
    audio_unit_host_object.al_source_primed = al_source_primed;
    if had_underrun && audio_unit_host_object.target_queued_buffers < 8 {
        audio_unit_host_object.target_queued_buffers += 1;
        log!(
            "AudioUnit {:?} increased queue target to {} buffers",
            audio_unit,
            audio_unit_host_object.target_queued_buffers
        );
    }
