...
use crate::frameworks::core_audio_types::{
    debug_fourcc, fourcc, AudioStreamBasicDescription, AudioStreamPacketDescription,
};
...
unsafe impl SafeRead for AudioFormatListItem {}
...
const kAudioFilePropertyFormatList: AudioFilePropertyID = fourcc(b"flst");
...
pub(super) fn property_size(property_id: AudioFilePropertyID) -> GuestUSize {
    match property_id {
        ...
        kAudioFilePropertyFormatList => guest_size_of::<AudioFormatListItem>(),
...
pub fn AudioFileGetProperty(
    ...
    match in_property_id {
        ...
        kAudioFilePropertyFormatList => {
            let asbd = AudioStreamBasicDescription::from_audio_description(
                host_object.audio_file.audio_description(),
            );
            let item = AudioFormatListItem {
                asbd,
                channel_layout_tag: 0,
            };
            env.mem.write(out_property_data.cast(), item);
        }
        ...
        kAudioFilePropertyMaximumPacketSize => {
            let maximum_packet_size: u32 = host_object.audio_file.packet_size_upper_bound();
            env.mem.write(out_property_data.cast(), maximum_packet_size);
        }
        ...
        kAudioFilePropertyPacketTableInfo => {
            ...
            let number_valid_frames = host_object
                .audio_file
                .packet_count()
                ...
                .try_into()
                .unwrap();
            let packet_table_info = AudioFilePacketTableInfo {
                number_valid_frames,
                priming_frames: 0,
                remainder_frames: 0,
            };
            env.mem.write(out_property_data.cast(), packet_table_info);
        }
        _ => unreachable!(),
    }
...
pub fn AudioFileReadBytes(
    ...
    if trace_audio_files() {
        log!(
            "AudioFileReadBytes trace: file={:?} start={} requested={} read={} out_buffer={:?}",
            in_audio_file,
            in_starting_byte,
            bytes_to_read,
            bytes_read,
...
fn AudioFileReadPacketData(
    ...
    if trace_audio_files() {
        log!(
            "AudioFileReadPacketData trace: file={:?} status={} bytes={} packets={} packet_descs={:?}",
            in_audio_file,
            status,
            if out_num_bytes.is_null() { 0 } else { env.mem.read(out_num_bytes) },
            if io_num_packets.is_null() { 0 } else { env.mem.read(io_num_packets) },
            out_packet_descriptions
        );
    }
    status
...
pub fn AudioFileReadPackets(
    ...
    return_if_null!(in_audio_file);
    return_if_null!(out_num_bytes);
    return_if_null!(io_num_packets);
    ...
    let frames_per_packet = host_object.audio_file.audio_description().frames_per_packet;
    ...
    let packets_read = if bytes_read == 0 {
        ...
        bytes_read.div_ceil(packet_size)
    };
    env.mem.write(io_num_packets, packets_read);
    if !out_packet_descriptions.is_null() {
        for packet_idx in 0..packets_read {
            let packet_start = packet_idx * packet_size;
            let data_byte_size = (bytes_read - packet_start).min(packet_size);
            let desc = AudioStreamPacketDescription {
                start_offset: i64::from(packet_start),
                variable_frames_in_packet: frames_per_packet,
                data_byte_size,
            };
            env.mem.write(
                out_packet_descriptions.cast::<AudioStreamPacketDescription>() + packet_idx,
                desc,
            );
        }
    }
    if trace_audio_files() {
        log!(
            "AudioFileReadPackets trace: file={:?} start_packet={} requested_packets={} packet_size={} bytes_read={} packets_read={} packet_descs={:?} status={}",
            in_audio_file,
            in_starting_packet,
            packets_to_read,
            packet_size,
