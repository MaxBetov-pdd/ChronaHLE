...
use crate::frameworks::core_animation::ca_eagl_layer::{
    find_fullscreen_eagl_layer, find_top_eagl_layer, get_pixels_vec_for_presenting, present_pixels,
};
use crate::frameworks::core_graphics::{CGRect, CGSize};
use crate::frameworks::foundation::ns_run_loop::pump_main_run_loop_audio_unit_sources;
...
pub(super) struct EAGLContextHostObject {
    ...
    rendering_api: EAGLRenderingAPI,
    ...
    presentation_resources: Option<PresentationResources>,
...
- (id)initWithAPI:(EAGLRenderingAPI)api sharegroup:(id)group {
    if !matches!(api, kEAGLRenderingAPIOpenGLES1 | kEAGLRenderingAPIOpenGLES2) {
        log!(
            "TODO: App requested EAGL initWithAPI:{} sharegroup:{:?}, returning nil as we only support API 1 and 2 for now",
            api,
            group
        );
        return nil;
    ...
    let mut gles_ins = create_gles_ctx_for_api(env, api);
...

    {
        ...
        host_obj.gles_ctx = Some(gles_ins);
        host_obj.rendering_api = api;
    }
...
- (id)initWithAPI:(EAGLRenderingAPI)api {
    if !matches!(api, kEAGLRenderingAPIOpenGLES1 | kEAGLRenderingAPIOpenGLES2) {
        log!(
            "TODO: App requested EAGL initWithAPI:{}, returning nil as we only support API 1 and 2 for now",
            api
        );
        return nil;
    }
...
- (())dealloc {
    let host_obj = env.objc.borrow_mut::<EAGLContextHostObject>(this);
    if let Some(resources) = host_obj.presentation_resources.take() {
        if let (Some(gles_ctx), Some(window)) = (host_obj.gles_ctx.as_mut(), env.window.as_mut()) {
            let mut gles = gles_ctx.make_current(window);
            unsafe {
                gles.DeleteFramebuffersOES(1, &resources.framebuffer);
                gles.DeleteTextures(1, &resources.texture);
            }
        }
    }
...
- (bool)renderbufferStorage:(NSUInteger)target
    ...
    let drawable = if drawable == nil {
        let fullscreen_layer = find_fullscreen_eagl_layer(env);
        if fullscreen_layer != nil {
            log!(
                "renderbufferStorage:{:?} fromDrawable:nil, using fullscreen CAEAGLLayer {:?}",
                target,
                fullscreen_layer,
            );
            fullscreen_layer
        } else {
            let top_eagl_layer = find_top_eagl_layer(env);
            if top_eagl_layer != nil {
                log!(
                    "renderbufferStorage:{:?} fromDrawable:nil, using top CAEAGLLayer {:?}",
                    target,
                    top_eagl_layer,
                );
                top_eagl_layer
    ...
    env.framework_state
        ...
        .insert(renderbuffer, env.options.scale_hack.get() != 1);
...
- (bool)presentRenderbuffer:(NSUInteger)target {
    ...
    // Unity and similar engines may render many frames without returning to
    // NSRunLoop. Keep RemoteIO fed at this reliable main-thread boundary.
    pump_main_run_loop_audio_unit_sources(env);
...
fn create_gles_ctx_for_api(env: &mut Environment, api: EAGLRenderingAPI) -> Box<dyn GLESContext> {
    match api {
        kEAGLRenderingAPIOpenGLES1 => create_gles1_ctx(env),
        kEAGLRenderingAPIOpenGLES2 => create_gles2_ctx(env),
        _ => unreachable!("unsupported EAGLRenderingAPI {api}"),
    }
}
...
unsafe fn present_renderbuffer(env: &mut Environment, context: id) {
    ...
    if use_native_gles2 {
        ...
        debug_assert!(gles_boxed.is_native_gles2());
        let (pixels, width, height) = read_renderbuffer(gles_boxed.as_mut(), Vec::new());
    ...
    // Reuse the hidden framebuffer and texture across frames. Creating and
    // deleting both objects for every present is particularly expensive with
    // an increased internal resolution.
    if resources.framebuffer == 0 {
        gles.GenFramebuffersOES(1, &mut resources.framebuffer);
    }
    let src_framebuffer = resources.framebuffer;
    gles.BindFramebufferOES(gles11::FRAMEBUFFER_OES, src_framebuffer);
    gles.FramebufferRenderbufferOES(
        gles11::FRAMEBUFFER_OES,
        gles11::COLOR_ATTACHMENT0_OES,
    ...
    if resources.texture == 0 {
        gles.GenTextures(1, &mut resources.texture);
    }
