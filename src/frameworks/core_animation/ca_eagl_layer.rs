...
/// Find the frontmost visible `CAEAGLLayer` in the current window hierarchy.
///
/// This is deliberately less strict than [find_fullscreen_eagl_layer]. It is
/// useful when an app passes nil as the drawable while its only practical
/// render target is already present in the layer tree, but its geometry is not
/// yet simple enough for the fullscreen fast path.
pub fn find_top_eagl_layer(env: &mut Environment) -> id {
    let windows = env.framework_state.uikit.ui_view.ui_window.windows.clone();
    let ca_eagl_layer_class: Class = msg_class![env; CAEAGLLayer class];
    ...
    for window in windows.into_iter().rev() {
        ...
        let eagl_layer = find_top_eagl_layer_in_tree(env, layer, ca_eagl_layer_class);
        if eagl_layer != nil {
            return eagl_layer;
        }
    }
...
fn find_top_eagl_layer_in_tree(env: &mut Environment, layer: id, ca_eagl_layer_class: Class) -> id {
    assert!(layer != nil);

    let (hidden, opacity, sublayers) = {
        let layer_host_obj: &CALayerHostObject = env.objc.borrow(layer);
        (
            layer_host_obj.hidden,
            layer_host_obj.opacity,
            layer_host_obj.sublayers.clone(),
        )
    };
    ...
    // Sublayers are stored back-to-front, so search from the visible front.
    for sublayer in sublayers.into_iter().rev() {
        let eagl_layer = find_top_eagl_layer_in_tree(env, sublayer, ca_eagl_layer_class);
        if eagl_layer != nil {
            return eagl_layer;
        }
    }
