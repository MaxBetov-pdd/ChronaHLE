...
impl ObjC {
    ...
    fn link_class_inner(
        ...
        let (default_class_superclass, default_metaclass_superclass) = if name == "NSObject" {
            (nil, class)
        } else {
            (
                self.get_class("NSObject", false, mem).unwrap(),
                self.get_class("NSObject", true, mem).unwrap(),
            )
        };
        let class_superclass = self
            .get_host_object(class)
            .and_then(|object| object.as_any().downcast_ref::<ClassHostObject>())
            .map_or(default_class_superclass, |class| class.superclass);
        let metaclass_superclass = self
            .get_host_object(metaclass)
            .and_then(|object| object.as_any().downcast_ref::<ClassHostObject>())
            .map_or(default_metaclass_superclass, |class| class.superclass);
        self.class_superclasses.insert(class, class_superclass);
        self.class_superclasses
            .insert(metaclass, metaclass_superclass);

        let name_ptr = mem.alloc_and_write_cstr(name.as_bytes()).cast_const();
        self.class_names.insert(class, name_ptr);
        self.class_names.insert(metaclass, name_ptr);
    ...
    pub fn register_bin_classes(&mut self, bin: &MachO, mem: &mut Mem) {
        ...
        for i in 0..(list.size / 4) {
            ...
            let class_t {
                superclass: metaclass_superclass,
                data: metaclass_data,
                ..
            } = mem.read(metaclass.cast());
            let class_rw_t {
                name: metaclass_name,
                ..
            } = mem.read(metaclass_data);

            self.class_names.insert(class, class_name);
            self.class_names.insert(metaclass, metaclass_name);
            self.class_superclasses.insert(class, class_superclass);
            self.class_superclasses
                .insert(metaclass, metaclass_superclass);

            let protocols = super::protocols::read_protocol_list(mem, base_protocols);
            if !protocols.is_empty() {
                self.class_protocols.insert(class, protocols);
            }
    ...
    pub fn register_bin_categories(&mut self, bin: &MachO, mem: &mut Mem) {
        ...
        for i in 0..(list.size / 4) {
            ...
            let protocols = super::protocols::read_protocol_list(mem, data.protocols);
            let class_protocols = self.class_protocols.entry(class).or_default();
            for protocol in protocols {
                if !class_protocols.contains(&protocol) {
                    class_protocols.push(protocol);
                }
            }
    ...
    pub fn class_is_subclass_of(&self, class: Class, superclass: Class) -> bool {
        ...
        let mut class = class;
        loop {
            let next = self.get_superclass(class);
            if next == nil {
                return false;
    ...
    pub fn get_superclass(&self, class: Class) -> Class {
        self.class_superclasses
            .get(&class)
            .copied()
            .unwrap_or_else(|| panic!("No runtime superclass metadata for class {class:?}"))
    }

    pub fn get_class_name_ptr(&self, class: Class) -> ConstPtr<u8> {
        self.class_names
            .get(&class)
            .copied()
            .unwrap_or_else(|| panic!("No runtime name metadata for class {class:?}"))
    }
...
pub(super) fn class_getName(env: &mut Environment, cls: Class) -> ConstPtr<u8> {
    if cls == nil {
        ...
        env.objc.get_class_name_ptr(cls)
    }
}
