...
impl super::ObjC {
    ...
    pub fn has_static_lifetime(&self, object: id) -> bool {
        let Some(entry) = self.objects.get(&object) else {
            panic!("No entry found for object {object:?}, it may have already been deallocated");
        };
        entry.refcount.is_none()
    }
    ...
    pub(super) fn clear_weak_references(&mut self, object: id, mem: &mut Mem) {
        if let Some(weak_locations) = self.weak_references.remove(&object) {
            for location in weak_locations {
                mem.write(location, nil);
            }
        }
    }
    ...
    pub fn dealloc_object(&mut self, object: id, mem: &mut Mem) {
        self.clear_weak_references(object, mem);
