...
fn write_return_this_wrapper(mem: &mut Mem, target: u32) -> GuestFunction {
    let ptr: MutPtr<u32> = mem.alloc(5 * 4).cast();
    mem.write(ptr + 0, 0xe92d4001); // push {r0, lr}
    mem.write(ptr + 1, 0xe59fc004); // ldr r12, [pc, #4]
    mem.write(ptr + 2, 0xe12fff3c); // blx r12
    mem.write(ptr + 3, 0xe8bd8001); // pop {r0, pc}
    mem.write(ptr + 4, target);
    let ptr = GuestFunction::from_addr_and_thumb_flag(ptr.to_bits(), false);
    assert!(!ptr.is_thumb());
    ptr
}

fn needs_return_this_wrapper(symbol: &str) -> bool {
    // Some iOS apps call old C++ constructor symbols as if they return `this`
    // in r0. The bundled dylib constructors may leave an internal value there,
    // so preserve the expected ABI at the import boundary. In Itanium mangling,
    // C1 and C2 are the complete-object and base-object constructors.
    symbol.starts_with("__Z") && (symbol.contains("C1E") || symbol.contains("C2E"))
}
...
pub struct Dyld {
    ...
    constants_to_link_later: Vec<(MutPtr<ConstVoidPtr>, &'static HostConstant, u32)>,
    non_lazy_host_functions: HashMap<&'static str, GuestFunction>,
    return_this_wrappers: HashMap<String, GuestFunction>,
    host_call_profile: Option<HostCallProfile>,
}

struct HostCallProfile {
    counts: Vec<u64>,
    last_report: Instant,
}
...
impl Dyld {
    ...
    fn profile_host_call(&mut self, idx: usize) {
        let Some(profile) = self.host_call_profile.as_mut() else {
            return;
        };
        if profile.counts.len() <= idx {
            profile.counts.resize(idx + 1, 0);
        }
        profile.counts[idx] += 1;

        if profile.last_report.elapsed() < Duration::from_secs(5) {
            return;
        }

        let mut hottest: Vec<_> = profile
            .counts
            .iter()
            .copied()
            .enumerate()
            ...
            .collect();
        hottest.sort_unstable_by_key(|&(_, count)| std::cmp::Reverse(count));
        echo!("Host API calls in the last profiling interval:");
        for (idx, count) in hottest.into_iter().take(20) {
            echo!("  {count:>10}  {}", self.linked_host_functions[idx].0);
        }
        profile.counts.fill(0);
        profile.last_report = Instant::now();
    }
    ...
    pub fn do_initial_linking(&mut self, bins: &[MachO], mem: &mut Mem, objc: &mut ObjC) {
        ...
        objc.register_bin_protocols(&bins[0], mem);
    ...
    fn do_non_lazy_linking(&mut self, bin: &MachO, bins: &[MachO], mem: &mut Mem, objc: &mut ObjC) {
        ...
        for &(ptr_ptr, ref name) in &bin.external_relocations {
            ...
            {
                // Custom constants require a complete Environment. Preserve
                // direct relocations as well as non-lazy symbol pointers so
                // embedded references such as a block literal's `isa` are
                // fixed up during late linking.
                self.constants_to_link_later
                    .push((ptr_ptr, template, offset));
                continue;
            } else {
        ...
        'ptr_loop: for i in 0..ptr_count {
            ...
            if let Some((_, template)) = search_host_dylibs(|dylib| dylib.constant_exports, symbol)
            {
                ...
                self.constants_to_link_later.push((ptr_ptr, template, 0));
                continue;
            }
    ...
    pub fn do_late_linking(env: &mut Environment) {
        ...
        for (symbol_ptr_ptr, template, offset) in to_link {
            ...
            env.mem.write(
                symbol_ptr_ptr,
                Ptr::from_bits(symbol_ptr.to_bits().wrapping_add(offset)),
            );
        }
    }
    ...
    fn do_lazy_link(
        ...
        for dylib in bins.iter() {
            if let Some(&addr) = dylib.exported_symbols.get(symbol) {
                let addr =
                    if dylib.name.starts_with("libstdc++.") && needs_return_this_wrapper(symbol) {
                        self.return_this_wrappers
                            .entry(symbol.to_owned())
                            .or_insert_with(|| write_return_this_wrapper(mem, addr))
